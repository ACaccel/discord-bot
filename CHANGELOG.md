...
- **`x-media-feed` — automatic X (Twitter) media-post forwarding.** A new plugin polls a configured list of X accounts and forwards their new **image / video** posts into a guild's feed channel. Only the account's own original posts are forwarded — reposts, replies (including the account's own thread continuations), and text-only posts are skipped. Each post is announced as a link on an embed proxy (default `fxtwitter.com`), so Discord unfurls a _playable_ video and the bot never downloads or re-uploads media. A per-handle database cursor makes the feed restart-safe: adding an account never backfills its existing timeline, a failed send is retried on the next pass rather than skipped, and nothing is double-posted; on every boot a reconciliation sweep also deletes cursors whose account was removed from the configuration, so the database stays an exact mirror of `accounts` (re-adding an account later starts from a fresh baseline). Shipped as `src/plugins/x-media-feed/` plus a new `src/infra/x-feed/` timeline-source Strategy and an `XFeedCursor` schema / repository, and loaded by the `nijika` personality. Timelines are read through a free third-party mirror rather than X's metered official API (Decision `0010`); the source is swappable behind an interface and `apiBaseUrl` can point at a self-hosted instance. **Operator action required:** the feature is opt-in and does nothing until configured — add an `x_media_feed` block with `"enabled": true` and an `accounts` list to `nijika`'s `config.json` (see the updated `config.example.json`), and add an `x_feed` channel id to that guild's `channels` map (a guild without it silently opts out). Adds one i18n key `replies:x_media_feed.post` and the `errors:x_feed.*` taxonomy (both locales).
...
- **`tomori` gains social-link preview, `/temp_role`, and a custom presence.** The public-facing `tomori` personality now loads the `social-link-preview` plugin (richer Twitter/X, Instagram, Threads, Facebook, Reddit, Bahamut, and Bilibili previews) and the `temp-role` plugin (the open-to-all `/temp_role` command), and registers a ready-time Discord presence so it no longer renders offline — the same interactive feature set as `nijika` minus the self-guild-only surfaces (the earthquake webhook and level-role sync). **Operator action required:** add `"temp_role"` to `tomori`'s live `config.json` `commands` list, add a `social_link_preview` block with `"enabled": true` (see the updated `config.example.json`), and re-run `yarn deploy -t tomori` to register `/temp_role`. Ensure tomori's Discord role holds `ManageRoles` (temp-role) and `ManageMessages` (the default `suppress` original-embed strategy). Adds one i18n key, `replies:tomori.presence_text` (both locales).

### Changed

- **`msg-archive`'s per-run backup transcript log is now opt-in and off by default.** The backup itself (DB ingest + debug-channel status) is unchanged and always runs; only the per-pass transcript file (`logs/backup/msg-archive-<guildId>-<stamp>.log`) is now gated, by a new `backup_log_enabled` flag in the `msg-archive` `config.json` (maps to `MessageBackupPluginConfig.backupLogEnabled`), defaulting to `false`. When off, `performBackup` writes through a `NullBackupLog` (Null Object) so no `logs/backup/` file or directory is created. **Operator action:** to keep writing transcript logs, add `"backup_log_enabled": true` to that bot's `config.json` (the shipped `config.example.json` defaults it to `false`). The current deployment config has been set to `false`.
- **Message edit/delete recording now records every channel locally; the `guild_events` rank ceiling gates Discord disclosure only.** Previously a channel above the `guild_events` rank ceiling (default `0`) was skipped entirely — no `event`-channel embed, no audit line, no attachment archival — so edits/deletes in private (rank-1+) channels left no record anywhere. The `guild-events` plugin now decouples the two concerns: the rank ceiling withholds **only** the Discord `event`-channel embed, while the local record runs **unconditionally** for every non-bot edit/delete regardless of rank — the structured audit line is written to `logs/<bot>/<guildId>/<YYYY-MM-DD>.log` (now also carrying stable `userId` / `channelId` / `messageId` correlation ids, plus edit attachment metadata) and deleted attachments are downloaded to `./data/deleted_attachments/<guildId>/`. The meaning of `permission_rank.features.guild_events.maxChannelRank` therefore narrows from "rank above which edits/deletes are **not recorded**" to "rank above which they are **not disclosed to the `event` channel**"; the config value is unchanged (default `0`) and no config edit is required. **Privacy note:** rank-1+ private-channel content and deleted attachments are now persisted to the bot host's file system, so anyone with file-system access can read them — this is the intended trade-off (Discord-side non-disclosure is preserved). Decision `0009`.
...
- **`social-link-preview` — Threads share links now preview, broken "Log in" cards are gone, X video previews play again, and the embed-proxy host lists moved out of the code into each bot's `config.json` (breaking).** Threads share short links (`https://www.threads.com/share/<token>/` — what the Threads app's share button emits) produced no preview at all, because only the `/@user/post/<id>` and `/t/<id>` forms were matched; a share link is now expanded to its canonical permalink before proxying, the same way Facebook share links already were. Threads needed more than the existing Facebook machinery, and the reason is not obvious: served a full desktop-browser user agent, Threads answers a share link with its client-side-routed application shell — HTTP 200, no redirect at all — so a server-side chase sees nothing to follow, and the expansion therefore uses a deliberately minimal user agent that still receives the plain redirect; and because the single-use share token is rejected on the follow-up request, the chase is bounced onward to an error page, so the permalink is read out of the **redirect chain** rather than from where the chase lands (that also absorbs the legacy `threads.net` → `threads.com` hop for free). The login-wall placeholders Threads and its embed proxies serve are now rejected in both Chinese and English: the wall carries a generic Instagram logo as its `og:image`, which previously scored as a usable image and let an ordinary Threads link post a broken "Log in" card. Every embed-proxy host was then re-probed with the same crawler user agent Discord itself sends, and the surviving hosts are no longer built into the code — the six `*ProxyHosts` lists are operator configuration now, because they name third-party services whose uptime changes far faster than a release ships. **Operator action required (breaking):** a bot whose `config.json` sets `social_link_preview.enabled` to `true` must carry all six lists — `twitterProxyHosts`, `instagramProxyHosts`, `threadsProxyHosts`, `facebookProxyHosts`, `redditProxyHosts`, `bilibiliProxyHosts` — and a missing one now fails startup with an error naming the key, rather than silently probing a built-in host that went dark months ago; the audited lists ship in each bot's `config.example.json`, and a disabled block still needs no host configuration at all. That audit also fixed two visible bugs: X video posts had been previewing as plain text cards, because `fxtwitter` no longer serves the Discord crawler any OpenGraph media tags at all, so the list now leads with `vxtwitter`, which does; and dead or ad-parked hosts (`vxthreads`, `rxddit`, `fixacebook`, `facebookez`) were spending a probe — and part of the per-link validation budget — on every single link. Preview quality gained a "weak image" tier for a related reason: some proxies answer a post that carries no media of its own with the author's profile avatar, and such a candidate now loses to any proxy holding the post's real image while still ranking above a title-only card. Finally, a legacy Facebook album permalink (`/<page>/photos/a.<album>/<photo>/`) now previews, normalised to the id-addressed `/photo/?fbid=…&set=…` form first — the proxies resolve the path form to an **unrelated person's post**, a preview well-formed enough that no content filter could have caught it. No new i18n keys.
...
- **Dependency advisories cleared: `axios` upgraded, four transitive packages re-resolved onto patched releases.** `axios` moves from `^1.15.2` to `^1.19.0`, clearing a HIGH advisory (`GHSA-gcfj-64vw-6mp9` — the Node HTTP adapter could reuse an inherited proxy after interceptor config cloning) plus nine moderate ones (prototype-pollution gadgets, `maxBodyLength` bypasses on `ReadableStream` / HTTP-2 uploads, a `NO_PROXY` bypass, and `formDataToJSON` recursion DoS). `brace-expansion`, `minimatch`, `js-yaml`, and `postcss` were re-resolved in `yarn.lock` to the newest release inside their existing semver ranges (`brace-expansion` 1.1.18 / 2.1.4 / 5.0.9, `minimatch` 3.1.5 / 5.1.9 / 9.0.9 / 10.2.6, `js-yaml` 4.3.1, `postcss` 8.5.25), clearing five further HIGH advisories without needing a `resolutions` override. The nine now-obsolete `minimatch` entries were dropped from the `audit-ci.jsonc` allowlist per its stated policy, and two new `tar` advisories (`GHSA-23hp-3jrh-7fpw` critical, `GHSA-8x88-c5mf-7j5w` high) were added to it: `@discordjs/node-pre-gyp` pins `tar@^6.1.11` and upstream ships fixes only on the 7.x line, so no in-range remediation exists — the existing install-time-only reachability rationale, and the plan to replace `@kirdock/discordjs-voice-recorder`, still stand. No source changes; no operator action required.
