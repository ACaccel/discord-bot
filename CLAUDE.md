...
- [`README.md`](README.md) — feature tour and the `.env` / `config.json`
  field reference
- [`CHANGELOG.md`](CHANGELOG.md) — one line per notable change
...
4. **No code change without its documentation.** A change to user-visible behaviour, a config field, a public contract, or a command must update every documentation surface it touches — `docs/architecture.md`, `README.md`, `CONTRIBUTING.md`, the matching `config.example.json`, and one line in `CHANGELOG.md` — in the same commit. A missing doc update is a defect, like a missing test.
