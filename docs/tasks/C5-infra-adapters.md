...
- [x] 新增 `isTransient(error: DatabaseError): boolean` helper，依
      ...
      不得各自重建此檔
- [x] `ConnectionManager.getConnection` 內部對 transient 失敗做**有上限的退避重試**
- [x] 重試耗盡或 persistent 失敗時，把該 `guildId` 標記為 disabled
- [x] `ConnectionManager` 在標記 guild disabled 時**自行生成 `traceId`**
      （原由 `BaseBot` boot 時 per-bot 產生）
- [x] 新增 `isDisabled(guildId)`，回傳 disabled 狀態與其 `traceId`
- [x] 在 C5 設計文件明示 **per-URI 共用範圍**：`ConnectionManager` 以 URI 為 key
      共用（`sharedConnectionManagers` map），`disabledGuilds` 成為內部狀態後，
      共用同一 URI 的 bot 會共享此 set
- [x] 確認 `traceId` 穿線一致——`requireGuildRepos`（C6）取得的 `traceId` 與
      結構化 log 中的一致
- [x] 補測試：以 `StaticConnectionManager` + 注入失敗，驗證 transient 重試與
      persistent 標記行為
