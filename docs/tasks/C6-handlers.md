...
- [ ] `requireGuildRepos` 的 disabled-guild 守衛（第 2 道）改讀
      `ConnectionManager.isDisabled(...)`，不再讀 `bot.disabledGuilds`
- [ ] 確認回 `errors:db.guild_disabled` 時附帶的 `traceId` 取自
...
- [ ] handler 指令名稱 / 選項描述改以 `commands` 命名空間 catalog key 取代
      handler 內的 CJK literal
- [ ] 移除 `src/handlers/` 內豁免指令 metadata 的 `// i18n-ignore` 註記
...
- [ ] 各 handler catch 改為「`logError(...)`（operator 通道）+
      `replyForError(..., 'replies:<feature>.failed')`（user 通道）」
- [ ] 保留各指令既有的 `replies:<feature>.failed` 語氣文案，作為非 `DomainError`
      回退用
- [ ] 補 handler 邊界錯誤對應測試：分別注入 `DomainError` 與 raw error，驗證
