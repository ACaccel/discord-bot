...
- [x] 待 C8 完成 D4 callsite 盤點後，評估 `src/utils/bot_cmd.ts`（含
      `buildCommandJsonBody`）的職責歸屬——`buildCommandJsonBody` 同時由
      `deploy.ts`（build 期）與 `handlers/commands/index.ts` 的 runtime command
      註冊路徑消費，其輸入型別 `LocalizedCommandConfig` 屬 handler 契約，
      裁定屬 runtime handler 邏輯
- [x] 依評估結論遷入 `scripts/` 或交還 C6，更新所有 import — 交還 C6，
      落於 `src/handlers/commands/command-builder.ts`，import 已更新
- [x] 若遷入 `scripts/`：維持 §1 邊界規則（`scripts/` 不參與 runtime）
      — 不適用（裁定不遷入 `scripts/`）
