...
- **C6 Handlers — `/roll_call`'s `users` option now expands role mentions.** The handler's `users` branch was rewritten around a new pure helper `src/handlers/commands/roll_call/resolve-targets.ts` (`parseRollCallMentions`, `resolveRollCallTargets`, `rollCallOutcomeReply`, `createGuildMemberSource`, `ROLL_CALL_MAX_TARGETS = 50`): the raw option string is now parsed for both user (`<@id>` / `<@!id>`) and role (`<@&id>`) mentions, each role is expanded to its non-bot members and the result merged + de-duplicated by member id. Because role expansion needs a full `guild.members.fetch()`, the branch now `deferReply`s to stay inside the 3s ack window (the `activity_id` branch is unchanged; the shared success reply is now ack-state aware). `@everyone` / `@here` (role id === guild id) is rejected and a final set over `ROLL_CALL_MAX_TARGETS` (50) is refused — the first specific failure wins over the cap. A discriminated `ResolveTargetsOutcome` behind an injected `MemberSource` keeps the resolver pure and unit-testable. Adds `replies:roll_call.{role_no_members,everyone_not_allowed,too_many_targets}` and rewords `replies:roll_call.format_error` + `commands:roll_call.options.users.description` (both locales). The reaction-handler responder is unaffected (the broadcast announcement still carries only expanded `<@id>` user mentions). Tests: a `resolve-targets` unit suite. Only the `users` option description changed, so re-run `yarn deploy` to refresh it.
- **C6 Handlers / C8 Plugins / architecture — the giveaway feature moved from option-heavy slash commands to modal and select-menu UI.** `/giveaway_create` drops its four options and instead `showModal`s a `giveaway_create` modal (text inputs for duration, prize, winner count, and description, built by `build-giveaway-modal.ts` mirroring `ai_settings`); a new `src/handlers/modals/giveaway_create/` modal handler delegates to the plugin's `handleGiveawayCreate`, which now reads `interaction.fields` and validates the winner count server-side (modals have no numeric input). `/giveaway_delete` drops its `message_id` option and instead lists active giveaways as a `giveaway_delete` string-select (paged at 25 options per row) via the plugin's new `handleGiveawayDeletePrompt`; a new `src/handlers/select-menus/giveaway_delete/` handler runs `handleGiveawayDeleteSelection` on pick. The announcement embed, the 🎉 entry, the scheduled draw, the schema, and the repository are unchanged. C8: giveaway plugin entry rewritten for the modal-and-select flow; architecture: the `GiveawayPlugin` row notes the modal-create and select-delete UI. Adds the giveaway modal label and placeholder keys plus `invalid_winner_num`, `no_active_giveaways`, `delete_select_placeholder`, and `delete_option_label` (both locales), and removes the giveaway commands' option-description keys. Tests: a `build-giveaway-modal` unit suite, the rewritten `handleGiveawayCreate` modal-field suite, and new `handleGiveawayDeletePrompt` and `handleGiveawayDeleteSelection` suites. Command shapes changed, so `yarn deploy` must be re-run.
- **C4 Persistence / C8 Plugins / architecture — added the `/temp_role` temporary-notification-role feature.** A new general-purpose `temp-role` plugin (`src/plugins/temp-role/`, the thirteenth) creates a permission-less, mentionable role via the open-to-all `/temp_role` command and posts a claim message whose button reuses the existing `toggle_role|<roleId>` custom id (so members self-claim / release it). The role auto-deletes after a selectable lifetime capped hard at 30 days (default 30) via a giveaway-style `JobManager` one-shot job, rebuilt on `onReady`; expiry comparisons use the injected `Clock`, and creation is rejected at Discord's 250-role-per-guild ceiling. C4: new `TempRole` schema + `MongoTempRoleRepo` (now eight repositories) wired through `SCHEMAS` / `DocByName`, `Repos` / `buildRepos`, and `connection-manager` `buildModels`. C8: heading from twelve to thirteen plugins with a `temp-role` entry. architecture: a `TempRolePlugin` row in the plugin registry. Loaded by the `nijika` personality (enable via `temp_role` in the bot's `config.json` `commands`). The shared `toggle_role` button's replies are now localised (`zh-TW` / `en`) instead of hardcoded English. Adds `commands:temp_role.*`, `replies:temp_role.*`, and `replies:toggle_role.*` catalog keys (both locales). Tests: temp-role plugin-internals + create-handler unit suites, a `MongoTempRoleRepo` integration suite, and a `toggle_role` button unit suite. Decision `0006`.
- **C5 Infra Adapters / C8 Plugins — the Bilibili link-preview provider now resolves `b23.tv` short links.** `providers/bilibili.ts` becomes a composed provider (like Facebook): `canHandle` also matches `b23.tv` / `www.b23.tv`, and `build` first expands the opaque short token to its canonical `(www.|m.)?bilibili.com/video/<BV|av>` URL via `OgClient.resolveCanonical` (a browser-UA, SSRF-guarded redirect chase), then proxies that through the shared validate-before-post loop, carrying the original short link as `sourceUrl`. A resolution that fails or does not land on a bilibili video page (live / dynamic / article / bangumi) is skipped silently (`ok(null)`); the resolved URL's `?p=` part selector is preserved and tracking query dropped. `resolveCanonical` gained an optional `provider` argument (default `'facebook'`) so its Err-rail errors are tagged with the calling source. **Also fixed a pre-existing wiring gap**: `social-link-preview`'s `plugin.ts` was not forwarding `config.bilibiliProxyHosts` to `createDefaultLinkPreviewRegistry` (all five other `*ProxyHosts` were), so an operator's configured bilibili proxy list was silently ignored and only the built-in default applied. Tests: bilibili unit cases (b23 `canHandle`, resolve-then-proxy, `?p=` carry-through, non-video / failed resolution → `null`, direct-link no-resolve), a contract case mirroring the Facebook share-link flow (browser-UA redirect → crawler-UA proxy probe), a registry-routing case, and a plugin test asserting every configured proxy-host list (incl. bilibili) reaches the registry factory. No new i18n keys (a rewrite provider posts a plain URL).
