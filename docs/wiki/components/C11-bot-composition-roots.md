...
- `src/bot/nijika/` — web-facing. Loads `createEarthquakePlugin({ port })`, which owns the Express `/discord/earthquake` route and per-guild broadcast. Also loads `createLlmAutoReplyPlugin(config.llm_auto_reply, { blockedChannels: config.blocked_channels })` — the self-hosted-LLM auto-reply (C8). The `llm_auto_reply` config block is optional and fully defaulted in code (disabled by default); `blocked_channels` is reused so those channels are excluded from auto-replies as well as logging.
...
- `llm_auto_reply` — optional block consumed by `nijika`'s `createLlmAutoReplyPlugin` (C8). All fields are defaulted in code (`src/plugins/llm-auto-reply/config.ts`, zod `.strict()`), so the block may be partial or omitted entirely: `enabled` (bool, default `false` — strictly opt-in), `probability` (0..1, default `0.05`), `messageCount` (N most-recent messages to require, default `5`), `windowSeconds` (the N must span ≤ this, default `30`), `cooldownSeconds` (minimum gap between consecutive automatic replies in a channel, default `30`; `0` disables; a `fatcat_reply` force-trigger skips it but still records it), `endpoint` (self-hosted chat URL, default a non-functional placeholder), `timeoutMs` (per-request, default `10000`). An unknown key fails fast at startup. `nijika`'s live `config.json` enables it and points `endpoint` at the deployment's self-hosted host; the trycloudflare quick-tunnel URL changes on restart, so it is operator-updated there rather than baked into code.
