...
- `ephemeral` mode uses `policy.visibilityCeiling(guildId, member roles, commandChannelId, parentId)` (so the command channel caps a privileged user) and checks `ViewChannel` for the member. `public` mode forces the ceiling to rank 0 and checks `@everyone`, so a high-clearance invoker never leaks private channels into a shared reply.
- Build the allowed-channel set by walking `guild.channels.cache`; a channel present only in archived data (deleted / uncached) is never added — fail-safe exclusion. Apply the filter before aggregation so excluded channels contribute nothing. Charts are rendered with `canvas` (see `discord-helpers`), and chart / table text is passed in pre-translated to keep the renderer CJK-free.
