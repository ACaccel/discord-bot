...
A command that surfaces aggregated guild data (message counts, rankings, traffic) must not reveal activity from channels the invoker cannot see. The pattern, realized by `/traffic` (whole-guild), `/traffic_me` (the invoker's own stats), and `/traffic_user` (a named target's stats), is a **dual filter**: a channel's data is included only when it clears BOTH the operator-defined rank ceiling AND the Discord-native `ViewChannel` check. `/traffic_user` separates the two subjects — the visible-channel set is built from the **invoker** (clearance + `ViewChannel`) while the stats focus the **target**, so the target's own clearance never widens the view and a target with no visible activity (including a non-member) yields the same neutral no-data reply. Command-agnostic primitives — the `canvas` chart renderers, the `visibility-filter`, window/bucket helpers, and (shared by the per-user pair `/traffic_me` + `/traffic_user`) the `options` parser, the per-user `aggregation-user`, and the `user-view` (a `keyPrefix` picks each command's `replies:*` namespace) — live in a shared, codegen-ignored module `src/handlers/commands/traffic-shared/` (no `index.ts`, so `gen-registry` does not treat it as a command); `/traffic` keeps its own guild-wide aggregation / overview / view, and each per-user command folder keeps only its `index`. This is the "lift shared logic when a second handler needs it" outcome (mirrors `discord-helpers.ts`).
