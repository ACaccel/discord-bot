...
export const err = <E extends DomainError>(error: E): Err<E> =>
  Object.freeze({ ok: false as const, error });

export const isOk = <T, E extends DomainError>(r: Result<T, E>): r is Ok<T> => r.ok;
export const isErr = <T, E extends DomainError>(r: Result<T, E>): r is Err<E> => !r.ok;

/** Transform the success value. The error channel is untouched. */
export const map = <T, U, E extends DomainError>(r: Result<T, E>, fn: (v: T) => U): Result<U, E> =>
  r.ok ? ok(fn(r.value)) : r;

/** Transform the error value. The success channel is untouched. */
export const mapErr = <T, E extends DomainError, F extends DomainError>(
  r: Result<T, E>,
  fn: (e: E) => F,
): Result<T, F> => (r.ok ? r : err(fn(r.error)));

/** Monadic bind. The downstream function may return a different success type. */
export const andThen = <T, U, E extends DomainError>(
  r: Result<T, E>,
  fn: (v: T) => Result<U, E>,
): Result<U, E> => (r.ok ? fn(r.value) : r);

/** Return the success value or a fallback. Never throws. */
export const unwrapOr = <T, E extends DomainError>(r: Result<T, E>, fallback: T): T =>
