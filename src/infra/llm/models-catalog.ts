...
/**
 * Injected per-provider API keys. The plugin composition root calls
 * {@link setProviderApiKeys} during init with the typed-Env-derived
 * map (same shape the registry holds). The previous implementation
 * read `process.env[PROVIDER_API_KEY_ENV[provider]]` directly, which
 * violated the project-wide no-restricted-syntax rule once this file
 * moved into `src/infra/llm/` (audit C-3 / PR-E E-4). Routing through
 * the typed Env keeps the rule honoured and removes the last direct
 * `process.env` read in infra.
 */
let injectedApiKeys: LlmProviderApiKeys = {};
export const setProviderApiKeys = (keys: LlmProviderApiKeys): void => {
  injectedApiKeys = keys;
};
...
async function fetchModels(provider: LLMProviderName): Promise<string[]> {
  const apiKey = injectedApiKeys[provider];
